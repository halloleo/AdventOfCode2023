...
;; The alist of what's in the bag
(setq full-bag
      (to-alist '((12 red) (13 green) (14 blue))))

(setq bag-keys
      (mapcar 'car full-bag))

(defun cube-possible (cube)
  "Test whether this cube draw (i.e. cube color plus cube number)  can
be taken from `full-bag'."
  (let* ((color (car cube))
        (num-cube (cdr cube))
        (num-bag (cdr (assq color full-bag))))
    (<= num-cube num-bag)))

(cube-possible '(blue . 3))
(cube-possible '(blue . 18))


(defun draw-possible (draw)
  (cl-every 'cube-possible draw))

(draw-possible (to-alist '((3 blue) (2 red))))


(defun draws-possible (draws)
  (cl-every 'draw-possible (draws-to-alists draws)))

(draws-possible '(((3 blue) (4 red)) ((1 red) (2 green) (6 blue)) ((2 green))))
(draws-possible (cadr (eval (car (car inp-data)))))


(defun count-for-game (game)
  "Calculate how much this game counts:
* `id` if all draws possiblw
* 0 else"
  (let ((id (car (eval game)))
        (draws (cadr (eval game))))
    (if (draws-possible draws)
        id
      0)))

(count-for-game (nth 2 inp-data))


(defun sum-of-possible (inp)
  "Get the sum of all game counts (but only the possible ones have non-zero
game counts)"
  (apply '+
         (mapcar 'count-for-game inp)))

(sum-of-possible sample)
