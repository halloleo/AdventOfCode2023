...
proc parseSeedsBlockAsSingleNumbers(blk: string): Seeds =
  let parts = blk.split ": "
...
var seedNums: seq[int]

# Parse the blocks: one Seed Block, rest are Map Blocks
var i = 0
for blk in blks:
  if i==0:
    seedNums = parseSeedsBlockAsSingleNumbers(blk)
  else:
...
proc lowestForSeedRange(startIdx:int, endIdx: int, oldLowest: int): int =
  result = oldLowest
  echo &"\nLowest for seeds {startIdx}..{endIdx} ({endIdx - startIdx})"
  for seed in startIdx..endIdx:
    let newLowest = seed2location(seed)
    if newLowest < result:
      result = newLowest

echo &"seedNums: {seedNums}"

var lowest = high(int)

for even in countup(0, len(seedNums)-1, 2):
  let startIdx = seedNums[even]
  let endIdx = seedNums[even] + seedNums[even+1] - 1
  lowest = lowestForSeedRange(startIdx, endIdx, lowest)
  echo &"Current lowest: {lowest}"
